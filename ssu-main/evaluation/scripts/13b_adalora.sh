...
# Make importlib.util.find_spec("bitsandbytes") return None and block imports
unset BNB_CUDA_VERSION
export TMPDIR=${TMPDIR:-/tmp}
BNB_BLOCK_DIR="$TMPDIR/disable_bnb_pkg"
mkdir -p "$BNB_BLOCK_DIR"
cat > "$BNB_BLOCK_DIR/sitecustomize.py" <<'PY'
import sys, importlib.util, importlib.abc

# Ensure find_spec reports "not found"
...
# Configs
export PYTHONPATH="$BNB_BLOCK_DIR:${PYTHONPATH}"
...
base_model_name="allenai/OLMo-2-1124-13B-Instruct"
if [[ -z "$lang_code" || -z "$checkpoint_steps" || -z "$postfix" ]]; then
    echo "Language code, checkpoint steps, and postfix are required arguments."
    echo "Usage: $0 <lang_code> <checkpoint_steps> <postfix>"
    exit 1
fi
...
# Add dummy tokens to avoid size mismatch issues
...
for task in "${tasks[@]}"; do
    ...
    lighteval accelerate \
        "model_name=${model_name},tokenizer=${model_name},adapter_weights=True,base_model=${base_model_name},batch_size=1,dtype=bfloat16" \
        "${task}" \
...
lm-eval --model hf \
    --model_args=pretrained=${base_model_name},tokenizer=${model_name},dtype=bfloat16,peft=${model_name} \
