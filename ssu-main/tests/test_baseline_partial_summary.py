...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'analysis'));sys.path.insert(0,str(R/'scripts/src'))
...
class Partial(unittest.TestCase):
 def test_incomplete_seeds_never_become_composite(self):
  with tempfile.TemporaryDirectory() as temp:
   ...
   write(root/'configs/ig_baseline_eval_v3.json',c);write(root/'configs/ig_baseline_repro_v1.json',{'models':{'full_fft':str(root/'fft'),'ssu_en_freeze50':str(root/'ssu')}});write(root/'configs/ig_baseline_train_v1.json',{'output_root':str(suite/'retrained'),'methods':{}});write(out/'protocol_lock.json',{'fixed':True})
   def add(seed):
   ...
   with patch.object(m,'R',root),patch('compare_baseline_history.generate'),patch('summarize_baseline_training.generate'):
