...
spec=importlib.util.spec_from_file_location('baseline_controller',R/'scripts/src/run_baseline_eval.py')
m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
class Contracts(unittest.TestCase):
 def setUp(self):self.c=json.loads((R/'configs/ig_baseline_eval_v3.json').read_text())
 def test_task_budget_and_seeds(self):
  jobs=m.jobs(self.c,False)
  self.assertEqual(len(jobs),11)
  self.assertEqual(len({(j['name'],j['seed']) for j in jobs}),11)
  self.assertEqual(sum(j['generative'] for j in jobs),6)
  for name in ['sum_en','sum_ig','mt_en2ig','mt_ig2en','ifeval','gsm8k']:
   self.assertEqual([j['seed'] for j in jobs if j['name']==name],[42])
